...
{
    class WhenTheParentsContainTwoHierarchicalBracketLayer
    {
        [Test]
        public void ThenTheChildContainsAMixtureOfTheGeneticCode()
        {
            ...
            Debug.Log(fRule);
            Assert.That(fRule, Is.EqualTo("+F[+F[+F+F]]").Or.EqualTo("+F[+F[-F-F]]").Or.EqualTo("+F[-F[+F+F]]").Or.EqualTo("+F[-F[-F-F]]")
                                    .Or.EqualTo("-F[+F[+F+F]]").Or.EqualTo("-F[+F[-F-F]]").Or.EqualTo("-F[-F[+F+F]]").Or.EqualTo("-F[-F[-F-F]]"));
        }
    }
}
